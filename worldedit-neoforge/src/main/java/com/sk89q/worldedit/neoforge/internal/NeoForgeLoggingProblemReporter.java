...
public final class NeoForgeLoggingProblemReporter implements ProblemReporter, AutoCloseable {
    private static final Logger LOGGER = LogManager.getLogger();

    public static <T> T with(Supplier<String> contextSupplier, Function<ProblemReporter, T> consumer) {
        try (var problemReporter = new NeoForgeLoggingProblemReporter(contextSupplier)) {
            return consumer.apply(problemReporter);
        }
    }

    NeoForgeLoggingProblemReporter(Supplier<String> contextSupplier) {
