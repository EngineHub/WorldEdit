...
public class Polygonal2DRegion extends AbstractRegion implements FlatRegion {
    ...
    public void expand(BlockVector3... changes) throws RegionOperationException {
        for (BlockVector3 change : changes) {
            if (change.getBlockX() != 0 || change.getBlockZ() != 0) {
                throw new RegionOperationException(translatable("worldedit.selection.polygon2d.error.expand-only-vertical"));
            }
    ...
    public void contract(BlockVector3... changes) throws RegionOperationException {
        for (BlockVector3 change : changes) {
            if (change.getBlockX() != 0 || change.getBlockZ() != 0) {
                throw new RegionOperationException(translatable("worldedit.selection.polygon2d.error.contract-only-vertical"));
            }
            int changeY = change.getBlockY();
            if (changeY > 0) {
                minY += changeY;
