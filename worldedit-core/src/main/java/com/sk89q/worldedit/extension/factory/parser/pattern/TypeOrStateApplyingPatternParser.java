    ...
    public Pattern parseFromInput(String input, ParserContext context) throws InputParseException {
        ...
        if (parts.length == 1) {
            ...
            for (String state : states) {
                if (state.isEmpty()) {
                    throw new InputParseException(translatable("worldedit.error.parser.empty-state"));
                }
                String[] propVal = state.split("=", 2);
                if (propVal.length != 2) {
                    throw new InputParseException(translatable("worldedit.error.parser.missing-equals-separator"));
                }
                final String prop = propVal[0];
                if (prop.isEmpty()) {
                    throw new InputParseException(translatable("worldedit.error.parser.empty-property"));
                }
                final String value = propVal[1];
                if (value.isEmpty()) {
                    throw new InputParseException(translatable("worldedit.error.parser.empty-value"));
                }
                if (statesToSet.put(prop, value) != null) {
                    throw new InputParseException(translatable("worldedit.error.parser.duplicate-property", text(prop)));
                }
            }
            if (type.isEmpty()) {
