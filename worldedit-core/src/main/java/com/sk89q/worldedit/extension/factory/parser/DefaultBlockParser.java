...
public class DefaultBlockParser extends InputParser<BaseBlock> {
    ...
    private static BaseBlock getBlockInHand(Actor actor, HandSide handSide) throws InputParseException {
        if (actor instanceof Player) {
            try {
                ...
                throw new InputParseException(translatable("worldedit.error.unknown", e.getRichMessage()), e);
            }
        } else {
                ...
                try {
                    ...
                    if (parts.length != 2) {
                        throw new InputParseException(
                                translatable("worldedit.error.parser.bad-state-format", text(parseableData))
                        );
                    }
                    ...
                    if (propertyKey == null) {
                        if (context.getActor() != null) {
                            throw new NoMatchException(translatable("worldedit.error.parser.unknown-property", text(parts[0]),
                                    text(type.getId())));
                        } else {
                    ...
                    if (blockStates.containsKey(propertyKey)) {
                        throw new InputParseException(translatable("worldedit.error.parser.duplicate-property", text(parts[0])));
                    }
                    Object value;
                    try {
                        value = propertyKey.getValueFor(parts[1]);
                    } catch (IllegalArgumentException e) {
                        throw new NoMatchException(translatable("worldedit.error.parser.unknown-value", text(parts[1]),
                                text(propertyKey.getName())));
                    }
                    ...
                    throw new InputParseException(translatable("worldedit.error.parser.bad-state-format", text(parseableData)));
                }
            }
        }
    ...
    private BaseBlock parseLogic(String input, ParserContext context) throws InputParseException {
        ...
        if (blockAndExtraData.length == 0) {
            throw new NoMatchException(translatable("worldedit.error.unknown-block", text(input)));
        }
        ...
        if (state == null) {
            ...
            if (stateStart == -1) {
                ...
                if (stateStart + 1 >= blockAndExtraData[0].length()) {
                    throw new InputParseException(translatable("worldedit.error.parser.hanging-lbracket", text(stateStart)));
                }
                int stateEnd = blockAndExtraData[0].lastIndexOf(']');
                if (stateEnd < 0) {
                    throw new InputParseException(translatable("worldedit.error.parser.missing-rbracket"));
                }
                stateString = blockAndExtraData[0].substring(stateStart + 1, blockAndExtraData[0].length() - 1);
            }
            if (typeString.isEmpty()) {
                throw new InputParseException(translatable("worldedit.error.parser.bad-state-format", text(blockAndExtraData[0])));
            }
            ...
            if ("hand".equalsIgnoreCase(typeString)) {
                ...
                try {
                    ...
                    throw new InputParseException(translatable("worldedit.error.incomplete-region"));
                }
        ...
        if (context.isRestricted()) {
            ...
            if (actor != null && !actor.hasPermission("worldedit.anyblock")
                ...
                throw new DisallowedUsageException(translatable("worldedit.error.disallowed-block", text(input)));
            }
        }
        ...
        if (DeprecationUtil.isSign(blockType)) {
            ...
            if (blockAndExtraData.length > 1) {
                ...
                if (ent == null) {
                    throw new NoMatchException(translatable("worldedit.error.unknown-entity", text(mobName)));
                }
                mobName = ent.getId();
                if (!worldEdit.getPlatformManager().queryCapability(Capability.USER_COMMANDS).isValidMobType(mobName)) {
                    throw new NoMatchException(translatable("worldedit.error.unknown-mob", text(mobName)));
                }
