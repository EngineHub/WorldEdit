...
public final class SpongeLoggingProblemReporter implements ProblemReporter, AutoCloseable {
    private static final Logger LOGGER = LogManager.getLogger();

    public static <T> T with(Supplier<String> contextSupplier, Function<ProblemReporter, T> consumer) {
        try (var problemReporter = new SpongeLoggingProblemReporter(contextSupplier)) {
            return consumer.apply(problemReporter);
        }
    }

    SpongeLoggingProblemReporter(Supplier<String> contextSupplier) {
