...
public final class CoreMcLoggingProblemReporter implements ProblemReporter, AutoCloseable {
    private static final Logger LOGGER = LogManager.getLogger();

    public static <T> T with(Supplier<String> contextSupplier, Function<ProblemReporter, T> consumer) {
        try (var problemReporter = new CoreMcLoggingProblemReporter(contextSupplier)) {
            return consumer.apply(problemReporter);
        }
    }

    CoreMcLoggingProblemReporter(Supplier<String> contextSupplier) {
