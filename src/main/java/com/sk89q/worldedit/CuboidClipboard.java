...
public class CuboidClipboard {
  ...
  public void rotate2D(int angle) {
    ...
    final BaseBlock newData[][][] =
        new BaseBlock[Math.abs(sizeRotated.getBlockX())][Math.abs(sizeRotated.getBlockY())][Math
    ...
    data = newData;
    size =
        new Vector(Math.abs(sizeRotated.getBlockX()), Math.abs(sizeRotated.getBlockY()),
   ...
   * @throws MaxChangedBlocksException
   */
  public int place(EditSession editSession, Vector pos, boolean noAir)
      throws MaxChangedBlocksException {
  ...
  public LocalEntity[] pasteEntities(Vector pos) {
    ...
    for (int i = 0; i < this.entities.size(); ++i) {
      ...
      if (copied.entity.spawn(copied.entity.getPosition().setPosition(
          copied.relativePosition.add(pos)))) {
