...
//        craftWorld.getHandle().tileEntityList.remove(te);
    ...
    public static net.minecraft.server.v1_6_R2.Block getNmsBlock(int type) {
        if (type < 0 || type >= net.minecraft.server.v1_6_R2.Block.byId.length) {
            return null;
        }
        return net.minecraft.server.v1_6_R2.Block.byId[type];
    }
    ...
    public static boolean isValidBlockType(int type) throws NoClassDefFoundError {
        return type == 0 || (type >= 1 && type < net.minecraft.server.v1_6_R2.Block.byId.length
                && net.minecraft.server.v1_6_R2.Block.byId[type] != null);
    }
}
